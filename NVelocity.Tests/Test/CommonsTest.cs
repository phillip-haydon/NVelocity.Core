...
namespace NVelocity.Test
{
	...
	public class CommonsTest : IDisposable
    {
		public void Dispose()
		{
		...
		{
			Assert.True(props.Count == 5, "expected to have 5 properties, had " + props.Count);

			Assert.True(props.GetString(prefix + "key").Equals("value"),
			              "key was not correct: " + props.GetString(prefix + "key"));

			// make sure the comma escaping is working correctly
			Assert.True(props.GetString(prefix + "commas.excaped").Equals("Hi, what'up?"),
			...
			Assert.True((o is ArrayList), prefix + "tokens_on_a_line was expected to be an ArrayList");
			Assert.True(((ArrayList) o).Count == 2, prefix + "tokens_on_a_line was expected to be an ArrayList with 2 elements");
			...
			Assert.True((o is ArrayList), prefix + "tokens_on_multiple_lines was expected to be an ArrayList");
			Assert.True(((ArrayList) o).Count == 2,
