...
namespace NVelocity
{
	...
	public class DuckTestCase
	{
		...
		{
			Assert.Equal("get invoked name", Evaluate("$duck1.name"));
			Assert.Equal("get invoked some", Evaluate("$duck1.some"));
			...
			Assert.Equal(expected.ToString(), duck2.ToString());
		}
		...
		{
			Assert.Equal("invoked some arg1 arg2 ", Evaluate("$duck1.some('arg1', 'arg2')"));
			Assert.Equal("invoked some arg1 ", Evaluate("$duck1.some('arg1')"));
			Assert.Equal("invoked some ", Evaluate("$duck1.some()"));
			Evaluate("$duck2.set()");
		...
		{
			Assert.Equal("invoked some message 1 message 2 ", Evaluate("$duck1.some($msg1, $msg2)"));
		}
		...
		public void Quoting()
		{
			...
			Assert.Equal("invoked some '" + amount + "' ", Evaluate("$duck1.some($test.Amount.to_squote)"));
			Assert.Equal("invoked some \"" + amount + "\" ", Evaluate("$duck1.some($test.Amount.to_quote)"));
			Assert.Equal("invoked some \"" + amount + "\" ", Evaluate("$duck1.some($test.Amount.to_quote)"));
			Assert.Equal("invoked some \"message 1\" \"message 2\" ", Evaluate("$duck1.some($msg1.to_quote, $msg2.to_quote)"));
			Assert.Equal("invoked some 'message 1' 'message 2' ", Evaluate("$duck1.some($msg1.to_squote, $msg2.to_squote)"));
		}
		...
		private string Evaluate(string toEvaluate)
		{
			...
			Assert.True(velocityEngine.Evaluate(c, sw, "eval1", toEvaluate));
